...
impl SourceConfigForSerialization {
    ...
    fn validate_and_build(self) -> anyhow::Result<SourceConfig> {
        ...
        match &self.source_params {
            SourceParams::GcpPubSub(_) | SourceParams::Kafka(_) | SourceParams::Nats(_) => {}
            _ => {
                if self.desired_num_pipelines > 1 || self.max_num_pipelines_per_indexer > 1 {
                    bail!("Quickwit currently supports multiple pipelines GCP PubSub, Kafka and NATS sources. Open an issue https://github.com/quickwit-oss/quickwit/issues if you need the feature for other source types.");
                }
            }
        }
